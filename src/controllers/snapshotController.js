...
const createSnapshotFromCSV = async (req, res) => {
  try {
    ...
    return res.status(403).json({
      ...
      error: 'Third-party CSV cannot be licensed. Only the data subject can grant consent in the Wallet app.',
    });
  } catch (error) {
    console.error('Error creating snapshot from CSV:', error);
    return res.status(500).json({ error: 'Failed to create snapshot from CSV' });
  }
};
