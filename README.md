...
- Error codes: `IDTOKEN_REQUIRED` (400), `IDTOKEN_INVALID`, `IDTOKEN_EXPIRED`, `IDTOKEN_ISSUER`, `IDTOKEN_AUDIENCE`, `IDTOKEN_SIGNATURE`, `IDTOKEN_VERIFIER_NOT_ALLOWED`, `IDTOKEN_KEY_NOT_PINNED`, `WALLET_NOT_IN_TOKEN` (401), `IDENTITY_CONFLICT` (409), `ORG_NOT_ALLOWED`, `ACCOUNT_DISABLED` (403), `IDTOKEN_UPSTREAM_UNAVAILABLE` (503: the Web3Auth JWKS could not be obtained (timeout, non-200, junk body or connection failure), so it is the identity provider's fault, never the token's; still fail closed; logged at error level as `idtoken_upstream_unavailable` with `jwksUrl` and `reason`; see `docs/TGE_SSO.md` §14). `ACCOUNT_DISABLED` (`User.disabledAt`) is also refused by `protect`, `authenticate`, password login and MCP tokens.
...
`SSO_TGE_ENABLED=true` 时必须是 `enforce`（§1）。日志永远不含 token。JWKS 取不到（超时、非 200、返回垃圾、连不上）时照旧
fail closed，统一回 `503 IDTOKEN_UPSTREAM_UNAVAILABLE`（"身份提供方暂时不可用"，不是"token 无效"），并打 error 级
`idtoken_upstream_unavailable`（`jwksUrl`、`reason`）；此时根本拿不到钥匙，pin 检查不会执行。
